  ...
  return (
    <div className="flex flex-col justify-center w-[96%] lg:w-[85%] m-auto mb-9 gap-5">
      ...
      <p className="border-l border-dotted border-red-500 ps-2">
        ...
        <span className="text-red-500">choosing the right tools</span> makes all
        the difference. As a communicator, web developer and designer, I have
        ...
        {cardsData?.map((element, index) => {
          return (
            <div key={index} className="Card w-[95%] sm:w-[60%] md:w-[45%] lg:w-[45%] xl:w-[25%] border-t-black border-t-4 border-l-4 border-l-black p-2">
              ...
              <div className="p-2 text-[1.1rem] flex flex-col gap-3 font-bold mt-2">
                {element?.option?.map((feature, index)=> {
                  return (
                    <div key={index}>
                      <span className="text-red-500 font-bold">&gt;</span> {feature}
                    </div>
                  );
                })}
              </div>
